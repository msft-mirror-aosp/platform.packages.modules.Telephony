...
import static com.android.qns.QnsConstants.THRESHOLD_EQUAL_OR_LARGER;
...
public class WifiQualityMonitor extends QualityMonitor {
    ...
    public static QualityMonitor getInstance(Context context) {
    ...
    private class WiFiThresholdCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onCapabilitiesChanged(
            ...
            handler.sendEmptyMessage(EVENT_WIFI_RSSI_CHANGED);
        }
    }
    ...
    private class WiFiStatusCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onLost(@NonNull Network network) {
            ...
            if (handler.hasMessages(EVENT_WIFI_NOTIFY_TIMER_EXPIRED)) {
                Log.d(mTag, "Stop all active backhaul timers");
                handler.removeMessages(EVENT_WIFI_NOTIFY_TIMER_EXPIRED);
                mWaitingThresholds.clear();
            }
            handler.sendEmptyMessage(EVENT_WIFI_STATE_CHANGED);
        }
    ...
    private WifiQualityMonitor(Context context) {
        ...
        handlerThread.start();
        handler = new WiFiEventsHandler(handlerThread.getLooper());
    ...
    private void handleMatchingThreshold(String key, Threshold th) {
        ...
        if (backhaul > 0) {
            ...
            if (!mIsBackhaulRunning) {
                ...
                handler.sendEmptyMessageDelayed(EVENT_WIFI_NOTIFY_TIMER_EXPIRED, backhaul);
    ...
    private void updateRequest(boolean register) {
        if (!register) {
            ...
            builder.setSignalStrength(mRegisteredThreshold);
            registerCallback();
            if (!mWifiStateIntentEnabled) {
    ...
    private void registerCallback() {
        ...
        if (!mIsRegistered) {
            ...
            mConnectivityManager.registerNetworkCallback(builder.build(), mWiFiThresholdCallback);
